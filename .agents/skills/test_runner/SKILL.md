...
| `HYDRATION_ERROR`    | `Text content does not match server-rendered HTML` in Next.js SSR               | Wrap browser-dependent UI in `useEffect` or dynamic import with `{ ssr: false }`, ensure identical date/locale formats across server and client.               |
| `NATIVE_MODULE_ERROR`| `Cannot find native module 'Expo...'` in React Native tests                      | Ensure missing Expo native module is mocked in `jest.setup.js` via `jest.mock('expo-...')`.                                                                    |
| `REACT_ACT_WARNING`  | `Warning: An update to Component inside a test was not wrapped in act(...)`      | Wrap state transitions in `await waitFor(...)` or `act(async () => { ... })`.                                                                                  |
| `QUEUE_CONCURRENCY`  | BullMQ / Redis job lock collision or worker process contention                   | Mock BullMQ queue `add` and worker `process` hermetically with in-memory callback resolution.                                                                   |
