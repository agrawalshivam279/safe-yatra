...
description: >-
  ...
  and multi-module build/test detection. Use whenever executing a complete atomic feature from start to finish, or calling /auto_cycle.
---
...
flowchart TD
    subgraph Phase1["Phase 1: Plan & Branch"]
        ...
        B --> C["Verify Git Clean ➔ Branch ➔ DB Safety & Infra Checklist ➔ Author Spec"]
    end
    ...
    subgraph Phase2["Phase 2: Code Synthesis"]
        ...
        E --> G["Write Service / UI / Schema (<= 320 LOC Backend / <= 500 LOC UI)"]
        F --> G
    end

    subgraph Phase3["Phase 3: Verify & Polish"]
        G --> H["Run /verify_step"]
        H --> I["Stage 0: Typecheck & Prisma ➔ Stage 0.5: API Contract Check ➔ Stage 1A: test_writer ➔ Stage 1B: test_runner"]
        I --> J{"🧪 Tests Pass & Contracts Valid?"}
        J -- "❌ No" --> K["🛑 HALT: Output Trace & Auto-Fix Prompt"]
        J -- "✅ Yes" --> L["🕵️ Stage 2: code_reviewer (Static Architecture, Security, DB Safety & Response Envelopes)"]
    end
...
| **3. Test & Contract Failure Gate**  | Any schema drift, lint error, or pytest / Jest assertion fails during verification.                                                                           | **Hard stop**. Does NOT review or ship. Outputs failure analysis and single-turn fix prompt.                                  |
...
| **5. Stage Timeout Circuit Breaker** | Any stage exceeds its time threshold: `plan_step` (5 min), Code Synthesis (10 min), `verify_step` (**12 min**), `ship_step` (**8 min** including CI polling). | **Interrupts hung process**, logs timeout diagnostic, and asks user to resume or inspect.                                     |

---

## 🌐 Phase-Aware Multi-Module Support

`auto_cycle` dynamically detects the target module and applies appropriate build, lint, and test commands:

| Module Target         | Compilation / Lint Command                | Verification Suite                        |
| :-------------------- | :---------------------------------------- | :---------------------------------------- |
| **`backend-spatial`** | `npx tsc --noEmit && npx prisma validate` | `npm test -- tests/<target>.test.ts`      |
| **`ml-risk-engine`**  | `ruff check .`                            | `pytest tests/test_<target>.py -v`        |
| **`mobile-app`**      | `npx tsc --noEmit`                        | `npm test -- __tests__/<target>.test.tsx` |
| **`admin-dashboard`** | `npx tsc --noEmit`                        | `npm test -- __tests__/<target>.test.tsx` |
...
4. Evaluates DB Migration Safety and authors the technical specification file in `<module>/docs/` or `docs/specs/`.
...
3. Adheres strictly to the LOC limits ($\le 320$ LOC Backend, $\le 500$ LOC UI) to avoid bloated commits.
...
2. Stage 0 runs module-specific typecheck, Prisma validation, and linter.
3. Stage 0.5 executes `api_contract_validator` (if REST routes/schemas touched).
4. Stage 1A spawns `test_writer` to author spec-driven unit/integration tests using the **Codebase Mock Catalog**.
5. Stage 1B spawns `test_runner` to execute the targeted test command (`npm test -- tests/<target>.test.ts` or `pytest tests/test_<target>.py -v`). For major milestone quality gates, verify full suite coverage via `npm run test:coverage` (backend) or `pytest --cov=app tests/` (ML).
6. If tests pass, runs `code_reviewer` (Stage 2) for static architectural, security, DB migration safety, response envelope, and performance alignment.
