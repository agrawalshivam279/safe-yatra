...
name: update-github
description: >-
  Commit changes, push to feature branch, create PR via GitHub MCP, squash merge to main,
  and clean up the local branch while keeping the remote branch on GitHub.
...
This custom workflow automates the end-to-end Git lifecycle for the Safe Yatra project: creating conventional commits under 100 characters, pushing the branch, creating a Pull Request, squash merging into `main`, deleting the local branch, and **preserving the remote branch on GitHub**.
...
- If on `main` and uncommitted changes exist:
  - Generate a descriptive branch name based on the changes (e.g. `feat/sos-sms-fallback` or `fix/danger-score-calc`).
...
- **User-Centric**: Describe what the change accomplishes or enables.

**Examples**:
- `feat: add offline sms fallback trigger for sos emergency dispatch` (65 chars)
...
- **PR Title**: Plain English feature/fix title without conventional prefix (e.g., `Add Offline SMS Fallback for SOS`).
...
## Verification
- <Brief explanation of testing or checks performed>
```
...
Merge the Pull Request into `main` using **Squash Merge** via GitHub MCP (`merge_pull_request`) or `gh pr merge --squash`.
...
5. **PR Failures**: If PR creation or check fails, stop immediately and report the error without attempting to merge.
